# 系统验证指南

表已重新创建，现在需要验证系统是否正常工作。

## 步骤 1: 检查服务状态

确保服务正在运行：

```bash
docker-compose ps
```

应该看到 `postgres` 和 `backend` 两个服务都在运行。

## 步骤 2: 验证表结构

在 DBeaver 中执行以下 SQL，检查表结构是否正确：

```sql
-- 查看表结构
SELECT 
    column_name,
    data_type,
    character_maximum_length,
    is_nullable,
    column_default
FROM information_schema.columns
WHERE table_name = 'alerts'
ORDER BY ordinal_position;
```

应该看到以下字段：
- id
- input
- enterprise_name
- time
- alert_type
- template_name
- om_type
- alert_key
- processed
- timeout_triggered
- created_at
- updated_at

## 步骤 3: 测试 API

### 方法一：使用测试脚本（推荐）

```bash
python test_api.py
```

### 方法二：使用 API 文档

1. 打开浏览器访问：`http://localhost:8000/docs`
2. 找到 `POST /api/alert` 接口
3. 点击 "Try it out"
4. 使用以下测试数据：

```json
{
  "input": "🔴 **【告警触发】监控告警**\n测试告警内容",
  "enterprise_name": "测试企业",
  "time": "2025-12-10 10:25:34",
  "alert_type": "告警触发",
  "template_name": "测试模板",
  "om_type": "ConnectionRate",
  "alert_key": "test_alert_001"
}
```

5. 点击 "Execute"
6. 应该返回创建的告警记录

### 方法三：使用 curl

```bash
curl -X POST "http://localhost:8000/api/alert" \
  -H "Content-Type: application/json" \
  -d '{
    "input": "测试告警内容",
    "enterprise_name": "测试企业",
    "time": "2025-12-10 10:25:34",
    "alert_type": "告警触发",
    "template_name": "测试模板",
    "om_type": "ConnectionRate",
    "alert_key": "test_alert_001"
  }'
```

## 步骤 4: 验证数据写入

在 DBeaver 中执行：

```sql
-- 查看最新创建的告警
SELECT * FROM alerts ORDER BY created_at DESC LIMIT 5;

-- 验证字段是否正确
SELECT 
    id,
    enterprise_name,
    alert_type,
    template_name,
    om_type,
    alert_key,
    time,
    created_at
FROM alerts
ORDER BY created_at DESC
LIMIT 1;
```

## 步骤 5: 测试查询 API

### 查询所有告警

```bash
curl http://localhost:8000/api/alerts
```

或在浏览器访问：`http://localhost:8000/api/alerts`

### 按企业查询

```bash
curl "http://localhost:8000/api/alerts?enterprise_name=测试企业"
```

### 按告警类型查询

```bash
curl "http://localhost:8000/api/alerts?alert_type=告警触发"
```

## 步骤 6: 测试超时检查逻辑

创建两个"告警触发"记录，验证超时检查逻辑：

### 创建第一个告警触发

```json
{
  "input": "第一个告警触发",
  "enterprise_name": "测试企业A",
  "time": "2025-12-10 10:00:00",
  "alert_type": "告警触发",
  "template_name": "模板A",
  "om_type": "TypeA",
  "alert_key": "alert_001"
}
```

### 创建第二个告警触发（相同企业、模板、OM类型）

```json
{
  "input": "第二个告警触发",
  "enterprise_name": "测试企业A",
  "time": "2025-12-10 10:05:00",
  "alert_type": "告警触发",
  "template_name": "模板A",
  "om_type": "TypeA",
  "alert_key": "alert_002"
}
```

第二个告警应该会"重置"第一个告警的超时检查。

## 步骤 7: 检查日志

查看后端服务日志，确认没有错误：

```bash
docker-compose logs -f backend
```

## 常见问题排查

### 问题 1: 表不存在

如果看到 "table alerts does not exist" 错误：

```bash
# 重启后端服务，让表自动创建
docker-compose restart backend

# 查看日志确认表已创建
docker-compose logs backend | grep -i "create"
```

### 问题 2: 字段不存在

如果看到 "column does not exist" 错误，说明表结构可能还是旧的。需要：

```bash
# 删除表
docker-compose exec postgres psql -U alert_user -d alert_db -c "DROP TABLE alerts;"

# 重启服务
docker-compose restart backend
```

### 问题 3: API 返回错误

检查：
1. 服务是否运行：`docker-compose ps`
2. 日志是否有错误：`docker-compose logs backend`
3. 请求数据格式是否正确（所有必填字段都已提供）

## 成功标志

✅ 服务正常运行  
✅ 表结构正确  
✅ 可以创建告警记录  
✅ 可以查询告警列表  
✅ 数据正确写入数据库  
✅ 日志无错误信息  

如果以上都正常，系统就配置完成了！



